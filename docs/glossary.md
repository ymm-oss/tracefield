...
本プロジェクトが検証する中核様式。**開放的探索を保ちながら**、provenance / reversibility / gateability を維持する。下記 B 群の構成要素を備える。Condition 5 に対応。
- 「半溶解性（semi-soluble）」: 入力やアクターの寄与が、完全に独立（不溶）でも完全に不可分（完全溶解）でもなく、**部分的に分離・撤回可能**であろうとする性質を指す（実験計画書の用法に基づく解釈）。
