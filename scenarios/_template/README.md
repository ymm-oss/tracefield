...
| `agents.json` | 視点（レンズ）の定義 | 3〜5体。`id`/`domain`/`desc`/`private_doc` ＋ 任意で **`model`**（エージェント別モデル=器官の偏り）と **`procedure`**（視点別方法論ファイル=手続きの偏り） |
...
- **手続きの偏り**: `procedure-<id>.md` を作り agents.json の `procedure` で指定（例: 現場主義/仮説駆動/検証主義/関係主義）。
- **器官の偏り**: agents.json の `model` でエージェント別モデル（例: 1体だけ `gemma4:26b`）。
- **来歴の偏り**: 既定で有効。run のたびに各エージェントの判断が `memory/<ID>.jsonl` に蓄積され、次回 `PRIVATE MEMORY` として注入される（`--memory false` で無効、`--memory-window N` で件数）。**蓄積するほど各視点のオリジナリティが育つ**。
